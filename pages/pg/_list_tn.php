	...
	<body>
		...
		<div class='masthead'>
			...
			<div class='col-md-5'>	
				...
				<?
					include "../db.php";
			 		$query = "Select (select num_doc from clients where id = client_id) num_doc, (select dover from clients where id = client_id) dover, (select id from clients where id = client_id) num_doc, (select UNP from clients where id = client_id) UNP from orders where number = ".$idAcct;
				...
				function addtn() {
					...
					if (i_err == 0){
						...
						if (num_doc != ''){
							...
							document.getElementById('fr').src = 'proc/tn.php?id='+idAcct+'&face_to='+face_to+'&to_face='+to_face+'&num_dover='+num_dover+'&date_dover='+date_dover+'&number='+number+'&seriass='+seriass+'&srt='+srt+'&flafff='+flafff+'&dates='+dates+'&no_nds='+no_nds;
							} else {
							document.getElementById('tn_err8').style.display = 'block';
						}
					...
					$('#confirm').on('click', function() {
						$.alertable.confirm('Внимание! При печати бланк сохраниться в базу! Если вы действительно уверены, что ввели все верно того подтвердите печать! Иначе отмените! Не забывайте проставить кол-во копий 2!!!').then(function() {
							...
							if (document.getElementById('chek').checked){
								...
								document.getElementById('fr').src = 'proc/tn_.php?id='+idAcct+'&face_to='+face_to+'&to_face='+to_face+'&num_dover='+num_dover+'&date_dover='+date_dover+'&number='+number+'&srt='+srt+'&flafff='+flafff+'&seriass='+seriass+'&dates='+dates+'&no_nds='+no_nds+'&is_not_exp='+is_not_exp;
