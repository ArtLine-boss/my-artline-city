...
$query = "select val from settings where id  = 4";
...
$query = "select c.CLIENT_NAME, c.ADDRESS_POST,c.PHONE_CITY,c.PHONE_MOB,c.EMAIL,c.ACCT,c.BANK,c.CODE_BANK,c.fio_dir1, c.num_doc_m from clients c  where c.ID = " . $clients;
$result = mysql_query($query) or die($query);
if ($row = mysql_fetch_row($result)) {
    $CLIENT_NAME = $row[0];
    $ADDRESS_POST = $row[1];
...
$pdf->AddPage();
// Logo
$pdf->Image('logoM.png', 140, 5, 45);

// Move to the right.
$pdf->SetFont('ArialMT', '', 9);
$pdf->Cell(0, 0, 'Изготовитель: Частное предприятие "Мечта клиента"', 0, 0);
$pdf->Ln();
$pdf->Cell(0, 7, '224030, г. Брест, ул. Карбышева, 74 ', 0, 0);
$pdf->Ln();
$pdf->Cell(0, 0, '8 (0162) 536828, 8 (029) 6094477, 8 (033) 6094477', 0, 0);
$pdf->Ln();
$pdf->Cell(0, 7, 'e-mail: ' . iconv("UTF-8", "cp1251", $user_mail));
$pdf->Ln();
$pdf->Cell(0, 0, 'УНП: 291546971');
$pdf->Ln();
$pdf->Cell(0, 7, 'Р/сч: BY14OLMP30123000990800000933');
$pdf->Ln();
$pdf->Cell(0, 0, 'Банк: ОАО "Белгазпромбанк"');
$pdf->Ln();
$pdf->Cell(0, 7, 'Код банка: ' . iconv("UTF-8", "cp1251", $bank_cod));
$pdf->Ln();
$pdf->Cell(0, 0, 'Адрес банка: г. Минск, ул. Притыцкого, 60/2');
...
while ($row = mysql_fetch_row($result)) {
    ...
    $html .= '<tr>' .
        ...
        . '<td align=right border=1111 nowrap >' . 'Без НДС' . '</td>'
...
$pdf->Ln(10);
$pdf->SetFont('ArialMT', '', 9);
$pdf->Cell(0, 0, 'Сумма НДС: Без НДС согласно  п.3, ст. 326 НК РБ');
$pdf->Ln();
...
// $pdf->Image('stamp.jpg',32,$var1,50);
...
$pdf->Ln(3);
//$pdf->Cell(100,5,'Директор _________________/ Мякишев Е. В.');
