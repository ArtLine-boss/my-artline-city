...
$wae = "select * from tn_list_par where num_tm =".$number." and seria = '".$seriass."'";
...
$query = "select user_id, client_id , DATE_FORMAT(DATE_OR, '%d %M %Y'),DATE_FORMAT(DATE_OR, '%d.%m.%Y'),CUR_ID from orders where number = ".$eid;
$result = mysql_query($query) or die($query);
if($row = mysql_fetch_row($result)){
	$users = $row[0];
	$clients = $row[1];
...
/*сведения о машине водителе*/
 ...
 if($flag_post == 1){
  ...
  $pdf->Cell(50,6,'ОДО "АртЛайнСити"',0,0);
 }
 ...
 $pdf->Cell(50,6,'ОДО "АртЛайнСити", 224030, г. Брест, ул. Карбышева, 74-9',0,0);
  $pdf->Ln();
...
<table width=100% cellpadding=0 celspacing=0>
	<tr>
		...
		<td colspan=6 valign=middle>Договор № '.iconv("UTF-8", "cp1251", $num_doc).', счет-протокол № '.$eid.' от '.$DATE_.'г.</td>
...
FOR ($z = 0; $z < count($strr); $z++){
	...
	IF ($str2[0] == $row[2] ){	
		...
		$qt = "INSERT INTO tn_list (prod_id, total, num_tn,order_id) VALUES ('".$str2[0]."', '".str_replace(',', '.', $str2[1])."', '".$number."',".$eid.");"; 
...
$table_html .= '	<tr>
		...
		<td border="1" bordercolor="#000" align="center">'.$nds.'</td>
		<td border="1" bordercolor="#000" align="center">'.number_format($sum_nds , 2, ',', ' ').'</td>
...
//<TD VALIGN=TOP COLSPAN=4 border=1111><FONT SIZE=5>Цена изготовителя: '.$price.'; опт.надбавка: 0%; предприятие-изготовитель-ОДО "АртЛайнСити";</FONT>
	...
	$table_html .= '<tr>
			...
			<td border=1 align=center  valign=middle bordercolor=#000>x</td>
			<td border=1  align=center valign=middle bordercolor=#000>'.number_format($sum_nds_all , 2, ',', ' ').'</td>
...
$tbl_row_w = 5;
// заголовок строка 1
$pdf->Cell($tbl_row_w*3,$tbl_row_h,'Операции','LT',0, 'C');
$pdf->Cell($tbl_row_w*4,$tbl_row_h,'Исполнитель', 'LT' ,0, 'C');
...
//строка номеров
$pdf->Cell($tbl_row_w*3,$tbl_row_h,' ','LB',0, 'C');
$pdf->Cell($tbl_row_w*4,$tbl_row_h,'12', 'LB' ,0, 'C');
...
$pdf->Ln();
// строки данных
$pdf->Cell($tbl_row_w*3,$tbl_row_h,'Погрузка','LB',0, 'C');
 $pdf->SetFont('ArialMT','', 6);
$pdf->Cell($tbl_row_w*4,$tbl_row_h,'ОДО "АртлайнСити"', 'LB' ,0, 'C');
 $pdf->SetFont('ArialMT','', 7);
$pdf->Cell($tbl_row_w*5,$tbl_row_h, ' ','LB',0, 'C');
$pdf->Cell($tbl_row_w,$tbl_row_h, ' ','LB',0, 'C');
$pdf->Cell($tbl_row_w*3,$tbl_row_h, $DATE_ORa,'LB',0, 'C');
$pdf->Cell($tbl_row_w*3,$tbl_row_h, $DATE_ORa,'LB',0, 'C');
$pdf->Cell($tbl_row_w*3,$tbl_row_h, ' ','LB',0, 'C');
...
$pdf->Cell($tbl_row_w*3,$tbl_row_h,'Разгрузка','LB',0, 'C');
 $pdf->SetFont('ArialMT','', 6);
$pdf->Cell($tbl_row_w*4,$tbl_row_h,'Грузополучатель', 'LB' ,0, 'L');
 $pdf->SetFont('ArialMT','', 7);
$pdf->Cell($tbl_row_w*5,$tbl_row_h, ' ','LB',0, 'C');
$pdf->Cell($tbl_row_w,$tbl_row_h, ' ','LB',0, 'C');
$pdf->Cell($tbl_row_w*3,$tbl_row_h, ' ','LB',0, 'C');
...
$pdf->Output();
	...
	IF ($flagggggggggggg){
$qt = "INSERT tn_list_par (order_id, num_tm, dolj_pol, fio_pol, num_dov, date_dol, user_log, date_,val_,flg,seria,dates_,exp_1c,type,summ,no_nds,pricep,put_list,POGRUZKA,RAZGRUZKA) VALUES
(".$eid.", '".$number."', '".$face_to."', '".$to_face."', '".$num_dover."', '".$date_dover."', '".$login."', '".$date_create."','".$str."',".$flafff.",'".$seriass."','".$_GET['dates']."',".$is_not_exp.",'ttn','".str_ireplace(",", ".", $summ_all)."','".$no_nds."','".$pricep."','".$list_put."','".$pogr."','".$razg."');"; 
