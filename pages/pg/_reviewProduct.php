...
<body>
   ...
   <div class='masthead'>
      <h3 class='text-muted'>Редактирование продукта <? echo " ".$PRODUCT_NAME ?></h3><br/>
		<div class='row'><div class='col-md-1'><div class='block1'><label>Наименование:</label></div></div><div class='col-md-2'><div class='block2'><input type='text' size = '100' name='productName' id = 'productName' <? echo "value='".$PRODUCT_NAME."'"?>></div></div></div>    
      <div class='row'><div class='col-md-1'><div class='block1'><label>Размер: </label></div></div><div class='col-md-2'><div class='block2'> <input type='text' name='' id ='productSize' <? echo "value='".$PRODUCT_SIZE."'"?>></div></div></div> 
		<div class='row'><div class='col-md-1'><div class='block1'>
			  <label>Сшивка : </label></div></div><div class='col-md-2'><div class='block2'>
			<?
			IF ($PRODUCT_Sd == '1'){
				echo '<input type="checkbox" name="selSh1" id = "selSh1" value="0" checked>';
			} else {
				echo '<input type="checkbox" name="selSh1" id = "selSh1" value="0">';
			}
			?>
		</div></div></div> 
		<div class='row'><div class='col-md-1'><div class='block1'><label>Сшивка: </label></div></div><div class='col-md-2'><div class='block2'>
			...
			switch ($PRODUCT_SH) {
			 ...
			 case '2':
				...
				<option value='2' selected>по длинной стороне</option>";
				  break;
			 default:
			...
			<?
			IF ($PRODUCT_SKOBA == '1'){
				echo '<input type="checkbox" name="skoba" id = "skoba" value="0" checked>';
			} else {
				echo '<input type="checkbox" name="skoba" id = "skoba" value="0">';
			}
			?>
		</div></div></div> 
		<hr>
...
window.onload=function(){
	...
	var PRODUCT_TEMPLATE = <? echo "'".$PRODUCT_TEMPLATE."'" ?>;
	PRODUCT_TEMPLATE_PART = PRODUCT_TEMPLATE.split("!")
	var kol = 0;
	for (var i = 0; i < PRODUCT_TEMPLATE_PART.length; i++) {
		PRODUCT_TEMPLATE_PART_NAME =  PRODUCT_TEMPLATE_PART[i].split("^");
		addRows();
	 kol++; 
		document.getElementById('textShab'+kol).value = PRODUCT_TEMPLATE_PART_NAME[0];
		PRODUCT_TEMPLATE_PART_OPER = PRODUCT_TEMPLATE_PART_NAME[1].split(";");
		for (var y = 0; y < PRODUCT_TEMPLATE_PART_OPER.length; y++) {
			kol++; 
			PRODUCT_TEMPLATE_PART_OPER_ONE = PRODUCT_TEMPLATE_PART_OPER[y].split("$");
			addRow();
			if (PRODUCT_TEMPLATE_PART_OPER_ONE[0] == '1'){
				$('#ch1'+ kol).prop('checked', true);
			}
			
			$('#selOper'+ kol).val(PRODUCT_TEMPLATE_PART_OPER_ONE[1]) 
			if (PRODUCT_TEMPLATE_PART_OPER_ONE[2] == '1'){
						$('#ch2'+ kol).prop('checked', true);
			}

			document.getElementById('kolOper' + kol).value = PRODUCT_TEMPLATE_PART_OPER_ONE[3];
			if (PRODUCT_TEMPLATE_PART_OPER_ONE[4] == '1'){
						$('#ch3' + kol).prop('checked', true);
			}
			//Оборудование
			 rts12('selOper' + kol,  'selEq' + kol, kol,PRODUCT_TEMPLATE_PART_OPER_ONE[5],PRODUCT_TEMPLATE_PART_OPER_ONE[8]);

			 if (PRODUCT_TEMPLATE_PART_OPER_ONE[6] == '1'){
				$('#ch4' + kol).prop('checked', true);
			}
			if (PRODUCT_TEMPLATE_PART_OPER_ONE[9] == '1'){
				$('#ch5' + kol).prop('checked', true);
			}

			document.getElementById('kolMat' + kol).value = PRODUCT_TEMPLATE_PART_OPER_ONE[10];
				if (PRODUCT_TEMPLATE_PART_OPER_ONE[11] == '1'){
				$('#ch6' + kol).prop('checked', true);
			}
			
			
			document.getElementById('kolstr' + kol).value = PRODUCT_TEMPLATE_PART_OPER_ONE[12];
				if (PRODUCT_TEMPLATE_PART_OPER_ONE[13] == '1'){
				$('#ch7' + kol).prop('checked', true);
			}
			document.getElementById('chSTRSize' + kol).value = PRODUCT_TEMPLATE_PART_OPER_ONE[14];
				if (PRODUCT_TEMPLATE_PART_OPER_ONE[15] == '1'){
				$('#chRas' + kol).prop('checked', true);
			}
			
			
			//PRODUCT_TEMPLATE_PART_OPER_ONE[16]

			
			//kol_str
			
			var node_id2 = document.getElementById("kol_str" + kol + "0");
				document.getElementById("kol_str" + kol ).remove();
				node_id2.getElementsByClassName('btn-group')[0].remove();
				node_id2.innerHTML = node_id2.innerHTML +  "<select data-style=" + '"btn-default btn-xs"' + " style='width: 100px;'  multiple name='kol_str[]" + kol + "'  id='kol_str" + kol + "' >" + document.getElementById('kol_str').innerHTML + "</select>" 
				
				if (PRODUCT_TEMPLATE_PART_OPER_ONE[16] != ''){
					strrr = PRODUCT_TEMPLATE_PART_OPER_ONE[16].split(",");
					
					for (var r = 0; r < strrr.length; r++) {
						$("#kol_str" + kol + " [value='"+ strrr[r] +"']").attr("selected", "selected");
	...
	function addRow()  // функция для добавления строки к части шаблона
		{
			...
			ce = ro.insertCell (-1);
			ce.innerHTML = "<h6 id = 'kol_str" + kol + "0'><select data-style=" + '"btn-default btn-xs"' + " style='width: 100px;'  multiple name='kol_str[]" + kol + "'  id='kol_str" + kol + "' >" + document.getElementById('kol_str').innerHTML + "</select>" ;
...
<script>
	function tabWalker(){
			...
			for (var i=1; i<rws.length; i++){ 							//цикл по всем строкам
		...
		str = str.substring(0, str.length-1);
		var post= '_updateProduct.php?productName='+productName+'&productSize='+productSize+'&productStr='+str+'&selSh='+selSh+'&skoba='+skoba_+'&productId='+productId+'&selSh1='+selSh1_;;
...
<script>
function rts3(id2, str, str2){


var theElement = document.getElementById(id2);
		theElement.innerHTML = "";
	$.get( '_list_mater1.php', { str: str, str2: str2}, function (data) {
		document.getElementById(id2).multiple = 'multiple';
		theElement.innerHTML = data;
...
<script>
function rts12(id1, id2, kol, str2,str3){
...
var theElement = document.getElementById(id2);
		...
		$.ajax({
			...
			rts1('selEq' + kol, 'optgroup' + kol,str3);
