...
<body>
	...
	//echo '<a href="_addAcct.php?id='.$idAcct.'">
        //  <button type="button" class="btn btn-primary"><i class="fa fa-reply fa-fw"></i> к заказу</button></a>' ?>
			 ...
			 <?
			 include "../db.php";
			 		$query = "Select  num_doc ,  dover ,  id , ADDRESS_POST from orders o, clients c where o.number = ".$idAcct." and c.id = o.client_id";
...
<div class="alert alert-danger" id = "tn_err1" style="display: none;">
							...
							<div class="alert alert-danger" id = "tn_err9" style="display: none;">
								...
								<strong>Ошибка!</strong> Введите пункт погрузки! <br/>
										</div>	
						
							<div class="alert alert-danger" id = "tn_err10" style="display: none;">
								<a href="#" class="close" id="close10">×</a>
								<strong>Ошибка!</strong> Введите пункт разгрузки! <br/>
							</div>
						...
						<div class="row">
							...
							<div class="col-md-3"><input type='checkBox' id='no_nds' name='no_nds'>	</div>
							</div>
						<div class="row">
						...
						<div class="row">
							<div class="col-md-5">Водитель</div>
							<div class="col-md-3"><input type="text" size = '40'   id="face_to" name="face_to" value = ""></div>
							</div>
							<div class="row">
								<div class="col-md-5">Автомобиль</div>
							<div class="col-md-3"><input type="text" size = '40' id="to_face" name="to_face" value = ""></div>
						</div>
						<div class="row">
	...
	<div class="row">
							<div class="col-md-5">Прицеп</div>
							<div class="col-md-3"><input type="text" size = '40'  id="pricep" name="pricep" value = ""></div>
							</div>
						<div class="row">
							<div class="col-md-5">Путевой лист(номер)	</div>
							<div class="col-md-3"><input type="text" size = '40'  id="list_put" name="list_put" value = ""></div>
							</div> 
					
						<div class="row">
							<div class="col-md-5">Пункт погрузки	</div>
							<div class="col-md-3"><input type="text" size = '40'  id="pogr" name="pogr" value = "г.Брест, ул. Карбышева, 74"></div>
							</div> 
					
						<div class="row">
							<div class="col-md-5">Пункт разгрузки		</div>
							<div class="col-md-3"><input type="text" size = '40'  id="razg" name="razg" value = "<? echo $ADDRESS_POST;?>"></div>
						</div> 
						<div class="row">
				...
				<table class='table  table-bordered' ><thead>
					<tr><td></td><td>Наименование продукта</td><td>Кол-во</td><td>Ед. Изм.</td><td>Цена</td><td>Цена без. Ндс</td><td>Сумма НДС</td><td>Цена с НДС</td><td>Кол-во мест</td><td>Масса</td></tr></thead>
				...
				$osr = abs($osr );
					$osr = round($osr,4);
						echo "<tr class='odd gradeX' id = $row[0] >";
							...
							echo "<td>$row[1]</td>";
							echo "<td> <input name='total$row[0]' id = 'total$row[0]' type='text' value='$osr' size='3'/></td>";
							echo "<td>$row[11]</td>";
							echo "<td>".ROUND($row[3],2)."</td>";
							echo "<td>".(ROUND(($row[4]/ $nds_),2))."</td>";
							echo "<td>".(ROUND($row[4],2) - ROUND(($row[4]/ $nds_),2))."</td>";
							echo "<td>".ROUND($row[4],2)."</td>";
							echo "<td> <input name='mest$row[0]' id = 'mest$row[0]' type='text' value='' size='3'/></td>";
							echo "<td> <input name='massa$row[0]' id = 'massa$row[0]' type='text' value='' size='3'/></td>";
						echo"</td></tr>";
					}
...
<script type="text/javascript">
 ...
 <script type="text/javascript">
  ...
  $("#close10").click(function(){
	document.getElementById('tn_err10').style.display = 'none';
...
function addtn() {
   ...
   if (document.getElementById('no_nds').checked) {
	...
	pogr = document.getElementById('pogr').value;
		if (pogr == ""){
		document.getElementById('tn_err9').style.display = 'block';
	i_err = i_err + 1;
	}
	razg = document.getElementById('razg').value;
		if (razg == ""){
		document.getElementById('tn_err10').style.display = 'block';
	i_err = i_err + 1;
	}
	...
	flafff = 0;	
     pricep = document.getElementById('pricep').value;
	list_put = document.getElementById('list_put').value;
	...
	if (array[i].checked) {
		...
		srt =	srt  + tr.id + "^" + document.getElementById('total'+tr.id).value  + "^" + document.getElementById('mest'+tr.id).value  + "^" +document.getElementById('massa'+tr.id).value  +  "|";
	...
	if (i_err == 0){
		...
		document.getElementById('fr').src = 'proc/ttn_view.php?eid='+idAcct+'&face_to='+face_to+'&to_face='+to_face+'&pricep='+pricep+'&list_put='+list_put+'&number='+number+'&seriass='+seriass+'&srt='+srt+'&flafff='+flafff+'&dates='+dates+'&no_nds='+no_nds+'&num_dover='+num_dover+'&date_dover='+date_dover+'&pogr='+pogr+'&razg='+razg;
		} else {
					document.getElementById('tn_err8').style.display = 'block';
		}
...
var 	i_err = 0;
  ...
  if (document.getElementById('no_nds').checked) {
	...
	if (i_err == 0){
		...
		document.getElementById('fr').src = 'proc/ttn_save.php?eid='+idAcct+'&face_to='+face_to+'&to_face='+to_face+'&pricep='+pricep+'&list_put='+list_put+'&number='+number+'&srt='+srt+'&flafff='+flafff+'&seriass='+seriass+'&dates='+dates+'&no_nds='+no_nds+'&num_dover='+num_dover+'&date_dover='+date_dover+'&pogr='+pogr+'&razg='+razg+'&is_not_exp='+is_not_exp;
