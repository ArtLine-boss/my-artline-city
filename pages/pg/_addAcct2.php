<?php
	...
	function fun_names($id)
	{
		...
		while ($cat = mysql_fetch_row($result)) {
			...
			if ($tn_count == '0') {
			echo ' <button type="button" class="btn btn-default"   id = "add">Добавить продукт</button>';
			}
			echo "</div> <br/>";
			echo "<table class='table  table-bordered' ><thead>
			<tr><td></td><td>Наименование продукта</td><td>Кол-во</td><td>Ед. изм.</td><td>Стоимость без НДС</td><td>Цена</td><td>Статус</td>			<td></td>					
			</tr></thead>";
			...
			if ($tn_count == '0') {
			if ($row[10] == "" OR $row[10] == "0" OR $admin == '4') {
			echo "<a onClick='_reviewAcctProduct1($row[0])'><button type= 'button' class='btn btn-default'><span class='glyphicon glyphicon-pencil'></span></button></a> ";
			if ($row[10] == "") {
			echo "<a onClick='_deleteAcctProduct($row[0],$idAcct)'><button type= 'button' class='btn btn-default'><span class='glyphicon glyphicon-trash'></span></button></a>";
			}
			} else {
			echo "<a onClick='_reviewAcctProduct2($row[0])'><button type= 'button' class='btn btn-default'><span class='glyphicon glyphicon-pencil'></span></button></a> ";
			}
			...
			<li><a onclick="func_mail(<? echo $idAcct ?>,<? echo $client_id ?>,'<? echo $email ?>')">Отправить
			на Email</a></li>
			<li><a href='proc/plan_job1.php?id=<? echo $idAcct ?>' target='_blank'>Бланк</a></li>
			...
			IF ($stat == 0) {
			echo '<button type="button" data-dismiss="modal" class="btn btn-default" onclick="get_job(' . $idAcct . ')">Отправить в работу</button></div>';
			}
			} ?>
			</div>
			</div>
         ...
         style="display: none;">
			...
			$query = "select id, eq_name, l_use, l_offset, ladnr, uandd, nadb_max,nadb_min,total_max,total_min, oper from equipment where id in (" . $srt_eq1 . ") and  eq_name is not null ORDER BY eq_name;";
			$result = mysql_query($query) or die($query);
			WHILE ($row = mysql_fetch_row($result)) {
			
			echo "<option value='$row[0]' " . ' style="display:block;"' . " data-nadb_max='" . $row[6] . "' data-nadb_min='" . $row[7] . "' data-total_max='" . $row[8] . "' data-total_min='" . $row[9] . "'>$row[1]</option>";
			
			$name_eq = $row[10];
			$query1 = "select  o.par, o.OPERATION_PRICE, o.id from operations o where FIND_IN_SET(o.id ,'" . $row[10] . "')";
			...
			$query = "SELECT s.SIZE ,e1.id  FROM size_print s, (select e.id, e.format from equipment e  WHERE e.id in (" . $srt_eq1 . ")) e1 WHERE FIND_IN_SET(s.ID,e1.format)";
			$result = mysql_query($query) or die($query);
			WHILE ($row = mysql_fetch_row($result)) {
			echo "<option value='$row[0]' data-attr = '$row[1]' " . ' style="display:none;"' . ">$row[0]</option>";
			}
			// }
			echo "</select>";
			?>
			...
			<option value='4' title="0.2|0.2|0.4|0.1" style="display:block;">мат+0</option>
			...
			<option value='9' title="0.6|0.6|1|0.5" style="display:block;"> СофтТач+мат</option>
			</select>
			</div>
         ...
         style="display: none;">
			...
			id='forms'>
			<div class='row'>
			<div class='col-md-3'><label>Наименование продукта: </label></div>
			<div class='col-md-9'><input type='text' id='p_names' size='80%' name='p_names' value=''
			...
			<div class='col-md-2'>
			<div class="form-group"><label>Переплет</label><br><label>Сторона переплета:</label>
			</div>
			</div>
			<div class='col-md-3'>
			...
			ro = tbl.rows[i++].insertCell(-1);
			ro.innerHTML = "<select id = 'p_color_" + kol_part + "' onblur=" + '"' + "get_price()" + '"' + " tabindex='" + tabkol + "'" + "> " + document.getElementById('p_color_i').innerHTML + "</select>";
			
			ro = tbl.rows[i++].insertCell(-1);
			ro.innerHTML = "<select class='chosen'  style='display: block;' id = 'p_mat_" + kol_part + "' onblur=" + '"' + "get_price()" + '"' + " tabindex='" + tabkol + "'" + " > " + document.getElementById('p_mat_i').innerHTML + "</select>	<input type='checkbox' " + "' onblur=" + '"' + "get_price()" + '"' + "value='' id='mat_firm" + kol_part + "' >Сырье заказчика &nbsp;&nbsp;&nbsp;<div id='sir" + kol_part + "' " + "></div>";
			...
			var price_oper = document.getElementById("p_color_" + i).options[y].title;
			}
			}
			var nadbv = 1.4;
			switch (nadb) {
			...
			seb_tir = (Number(price_oper) * Number(tir_list)) * Number(new_price);
			...
			switch (nadb) {
			case "2":
			price_lam = Number(val_mat[0]) * kol_pech_i;
			break;
			case "3":
			price_lam = Number(val_mat[1]) * kol_pech_i;
			break;
			case "5":
			price_lam = Number(val_mat[3]) * kol_pech_i;
			break;
			default:
			price_lam = Number(val_mat[2]) * kol_pech_i;
			break;
			}
			}
			}
			...
			*/
			...
			document.getElementById('fr').src = 'proc/act_.php?id=' + idAcct;
			...
			document.getElementById('fr').src = 'proc/act.php?id=' + id;
			$('#myacct').modal('show');
			}
			function addshamp() {
			$("#multiform").submit();
			...
			alert("Не введено наименование продукта!")
			return
			}
			...
			*/
			...
			var name_prod = document.getElementById('p_names').value;
			var unit_prod = 'шт.';
			...
			function slc() {
			var orderAcct = "<? echo $idAcct ?>";
			location.href = '_upStatusDoc.php?id=' + orderAcct;
			}
			function funee() {
			...
			var productKol = document.getElementById('productKol').value;
			...
			current = document.getElementById('par' + kol).value;
			str = str + current;
			str = str.concat("$");
			
			current = document.getElementById('optgroup' + kol).value;
			str = str + current;
			str = str.concat("!");
			kol++;
			}
			...
			var post = '_usl.php?productName=' + productName + '&productSize=' + productSize + '&productStr=' + str + '&productKol=' + productKol + '&productPrice=' + productPrice + '&id=' + id;
			...
			document.getElementById('kolss').value = kol;
			document.getElementById('namess').value = name;
			$('#myModald').modal('show');
			
			
			}
			function ref1(kol, name) {
			...
			p_cut2222 = 0
			if (document.getElementById('p_cut2_' + num_).checked) {
			p_cut2222 = 1;
			}
			//	pers = $('#pers' + num_).val();
			pers222 = 0
			...
			document.getElementById('p_names').value = shab[2];
			$('#p_fast').val(shab[3]);
			...
			function func_mail(id_order, id_client, mail) {
			...
			function post_mail(id_order, id_client, mail) {
			$.ajax({
			type: "GET",
			url: 'proc/acct_prot1.php',
