<?
	...
	$homepage = file_get_contents('https://webservices.belpost.by/searchRu/'.$track );
	...
	$pos = strpos($tb, '<b>Офис</b></font></th>');
	$tb = substr($tb , $pos + 27);
	$pos = strpos($tb, '<b>Событие</b></font></th>');
	$tb = substr($tb , $pos + 34);
	$tb = ltrim($tb); 
	$tb =  str_replace("\r\n", "", $tb);
	$tb =  str_replace("\t", "", $tb);
	...
	case '1':	
	switch ($admin) {
	case '4': $userStatus = "1,11,10,12,20"; break;
	case '5': $userStatus = "10"; break;
	case '6': $userStatus = "12"; break;
	case '7': $userStatus = "11"; break;
	...
	$z = $i + 1;
	$part = explode("|", $temp[$i]);
	IF ($part[3] != "" AND  $part[3] != '0' AND  $part[3] != 'Выберите') {  $eqq .= $part[3]." ".$part[4];	}
	IF ($part[6] != "" AND  $part[6] != '0') { $mat = explode(":",$part[6]); $mat_eqq.= $mat[0].' '.$part[5].' Кол-во листов : '.$mat[1];	}
	}
	}
	...
	if ($admin == '4'			){ echo	"<option value= '0' >Открыть</option><option value= '20' >Менеджер</option>";  }
	echo	"					</select>
	</div>
	</div>";
	echo '<div class="row">
	...
	case '9':
	$json = Array();
	$query="select  m.ID, m2.MT_TYPE, m.M_NAME , m.M_UNIT , m.M_SIZE ,m.M_PRICE ,m.M_AVA , m.M_KOL_ALL, (select km.title from kl_mat km where km.id = m.id_tree LIMIT 1) usel from material_attr m, material m2 where m.ID_M = m2.ID and m.arh = 0			";
	$result = mysql_query($query) or die($query);
	while ($row = mysql_fetch_row($result)) { 
	$inp = "<input type='checkbox' name='chjob' value='".$row[0]."'>";
	$json[] = array(
	
	'edit' =>"<a onClick='edit_mat($row[0])'><i class='fa  fa-edit' aria-hidden='true'></i></a>",
	...
	$max_num++;
	$query1 = "INSERT INTO order_product (ORDER_ID, PRODUCT_ID, TOTAL, PRICE, SUMM, TEMPLATE, TEMP_PR, SIZE, DIZ, SHABLON_CL, p_names, PR, press_diz, print_diz, sum_press, view_diz, view_press, flags, cshivka, cl_file, units, dates_rdy, fast,num_prod_ord,add_date) VALUES(	'{$insert_ID_ord}','{$row[1]}','{$row[2]}','{$row[3]}','{$row[4]}','{$row[5]}','{$row[6]}','{$row[7]}','{$row[8]}','{$row[9]}','{$row[10]}','{$row[11]}','{$name_dir3}','{$name_dir2}','{$row[14]}','{$row[15]}','{$row[16]}','4','{$row[18]}','{$name_dir1}','{$row[20]}','{$row[21]}','{$row[22]}','{$max_num}' , '{$orderDate}');";
	mysql_query($query1) or die($query1);
	}
	...
	// TTN
	case '37':	
	$query="select DATE_FORMAT(dates_, '%d.%m.%Y') dates_, num_tm, order_id, (select user_fio from users where user_login = user_log), del, exp_1c, type , 
	(select c.UNP from clients c where c.ID = (select client_id from orders o where o.NUMBER = order_id)) unp, id, summ from tn_list_par";
	...
	$opl = "";
	if ($admin == '4' OR $admin == '2' OR $login == '026' OR $login == '027' or $login == '032'){
	...
	$list_prod  = "";
	switch ($row[5]){
	case '4': $list_prod  = "<span class='label label-danger '>Брак/Отказ</span>"; break;	
	case '20': $list_prod  = "<span class='label label-danger '>Возврат</span>"; break;	
	case '0': $list_prod  = "<span class='label label-danger '>Возврат</span>"; break;	
	case '1': $list_prod  = "<span class='label label-info '>в цеху</span>"; break;	
	case '2': $list_prod  = " <span class='label label-success'>Готово</span>"; break;	
	case '3': $list_prod  = " <span class='label label-success'>Отдано</span>"; break;	
	case '21': $list_prod  = " <span class='label label-danger'>Cогласование с клиентом</span>"; break;	
	case '10': if($row[7] == '1') {
	$list_prod = " <span class='label label-primary'>Дизайн ".$row[8]."</span>"; break;	
	} else {
	$list_prod =  "<span class='label label-primary '>Ожидание дизайна</span>"; break;	
	} break;	
	case '11': if($row[7] == '1') {
	$list_prod =  "<span class='label label-primary '>Препресс ".$row[8]."</span>"; break;	
	} else {
	$list_prod =  "<span class='label label-primary '>Ожидание препресса</span>"; break;	
	}
	case '12': if($row[7] == '1') {
	$list_prod = " <span class='label label-info'>Печатает ".$row[8]."</span>"; break;	
	} else {
	$list_prod =  " <span class='label label-info'>Ожидание печати</span>"; break;	
	}
	default:  $list_prod =  "";
	break;	
	}
	...
	<div class='col-md-1'>";
	switch ($rw[6]){
	case 4: $list_prod = $list_prod. "<span class='label label-danger '>Брак/Отказ</span>"; break;	
	case 21: $list_prod  = $list_prod." <span class='label label-danger'>Cогласование с клиентом</span>"; break;	
	//case 0: $list_prod  = $list_prod. "<span class='label label-danger '>Созданный</span>"; break;	
	case 3: $list_prod = $list_prod. "<span class='label label-success '>Отдано заказчику</span>"; break;	
	case 20: $list_prod = $list_prod. "<span class='label label-danger '>Возврат</span>"; break;	
	case 1: $list_prod = $list_prod. "<span class='label label-info '>в цеху</span>"; break;	
	case 2: $list_prod = $list_prod. " <span class='label label-success'>Готово</span>"; break;	
	case 10: $list_prod = $list_prod. " <span class='label label-primary'>Дизайн</span>"; break;	
	case 11: if($rw[7] == '1') {
	$list_prod = $list_prod. "<span class='label label-primary '>Препресс</span>"; break;	
	} else {
	if($rw[7] == '0') {
	$list_prod = $list_prod. "<span class='label label-primary '>Ожидание препресса</span>"; break;	
	}
	else {
	$list_prod = $list_prod. ""; 
	}
	...
	case 12: if($rw[7] == '1') {
	$list_prod = $list_prod. " <span class='label label-info'>Печатается</span>"; break;	
	} else {
	if($rw[7] == '0') {
	$list_prod = $list_prod. " <span class='label label-info'>Ожидание печати</span>"; break;	
	}
	else {
	$list_prod = $list_prod. ""; 
	}
	}
	default: $list_prod = $list_prod. "";  break;	
	}
	...
	IF ($part[6] != "" AND  $part[6] != '0') { $mat = explode(":",$part[6]); $list_prod.=  "<div class = 'row'><div class='col-md-2'>Бумага: </div><div class='col-md-10'>".$mat[0].' '.$part[5]."</div> </div> <div class = 'row'><div class='col-md-2'>Кол-во листов : </div><div class='col-md-4'> ".$mat[1].'</div> </div>';	}
	...
	if ($login == 'admins' ){
	$query="select 
	DATE_FORMAT(op.dates_rdy, '%d.%m.%Y ') dt,
	op.ORDER_ID , 
	(select (select c.CLIENT_NAME from clients c where c.ID = o.CLIENT_ID ) from orders o where o.number = op.ORDER_ID) name,
	op.status,
	(IF((select flags from lock_task lt where lt.id_prod = op.id and lt.oper = op.status ORDER BY lt.id DESC LIMIT 1) IS NULL, 0, (select flags from lock_task lt where lt.id_prod = op.id and lt.oper = op.status ORDER BY lt.id DESC LIMIT 1))) flag,
	...
	case 21: $list_prod  = " <span class='label label-danger'>Cогласование с клиентом</span>"; break;	
	case 10: if($row[4] == '1') {
	$list_prod = " <span class='label label-primary'>Дизайн ".$row[13]."</span>"; break;	
	} else {
	$list_prod =  "<span class='label label-primary '>Ожидание дизайна</span>"; break;	
	} break;	
	case 11: if($row[4] == '1') {
	$list_prod =  "<span class='label label-primary '>Препресс ".$row[13]."</span>"; break;	
	} else {
	$list_prod =  "<span class='label label-primary '>Ожидание препресса</span>"; break;	
	}
	case 12: if($row[4] == '1') {
	$list_prod = " <span class='label label-info'>Печатает ".$row[13]."</span>"; break;	
	} else {
	$list_prod =  " <span class='label label-info'>Ожидание печати</span>"; break;	
	}
	default: $list_prod = $list_prod. "";  break;	
	...
	$list_prod2 = '';
	switch ($row[3]){
	case 4: $list_prod2 = $list_prod2. "Брак/Отказ"; break;	
	case 0: $list_prod2  = "Возврат"; break;	
	case 21: $list_prod2  = "Согласование"; break;	
	case 20: $list_prod2  = "Возврат"; break;	
	case 1: $list_prod2  = "в цеху"; break;	
	case 2: $list_prod2  = " Готово"; break;	
	case 10: $list_prod2 = " Дизайн"; break;	
	case 11: if($row[4] == '1') {
	$list_prod2 =  "Препресс"; break;	
	} else {
	$list_prod2 =  "Ожидание препресса"; break;	
	}
	case 12: if($row[4] == '1') {
	$list_prod2 = " Печатается"; break;	
	} else {
	$list_prod2 =  "Ожидание печати"; break;	
	}
	default: $list_prod2 =  "";  break;	
	}
	...
	switch ($cshivka[0]) {
	case '1': $list_prod3.= 'пружина 6,4 мм;'; break;
	case '2': $list_prod3.=  'пружина 8,0 мм;'; break;
	case '3': $list_prod3.=  'пружина 9,5 мм;'; break;
	case '4': $list_prod3.=  'пружина 11,0 мм;'; break;
	case '5': $list_prod3.=  'пружина 12,7 мм;'; break;
	case '6': $list_prod3.=  'пружина 14,3 мм;'; break;
	case '7': $list_prod3.=  'скоба;'; break;
	case '8': $list_prod3.=  'Твердая обложка (PUR);'; break;
	case '9': $list_prod3.=  'Твердая обложка (скобы);'; break;
	case '10': $list_prod3.=  'Твердая обложка;'; break;
	case '11': $list_prod3.=  'Твердая обложка (пружина);'; break;
	case '12': $list_prod3.=  'термоклей;'; break;
	case '13': $list_prod3.=  'нитка;'; break;
	...
	IF ($part[0] != "" AND  $part[0] != '0') { $list_prod.=  " ".$part[0].";";	} else { $list_prod.= "; ";}
	IF ($part[1] != "" AND  $part[1] != '0') { $list_prod.=  " ".$part[1].";";	} else { $list_prod.= "; ";}
	IF ($part[2] != "" AND  $part[2] != '0') { $list_prod.=  " ".$part[2].";";	} else { $list_prod.= "; ";}
	IF ($part[3] != "" AND  $part[3] != '0' AND  $part[3] != 'Выберите') { $list_prod.=  "".$part[3].';  '.$part[4].";";	}  else { $list_prod.= "; ;";}
	IF ($part[6] != "" AND  $part[6] != '0') { $mat = explode(":",$part[6]); $list_prod.=  "".$mat[0].' '.$part[8].' ;'.$mat[1].";";	} else { $list_prod.= ";; ";}
	IF ($part[7] != "" AND  $part[7] != '0') { $list_prod.=  "YES ;";	} else { $list_prod.= "NO; ";}
	IF ($part[8] != "" AND  $part[8] != '0') { $list_prod.=  $part[8].";";	} else { $list_prod.= "; ";}
	IF ($part[9] != "" AND  $part[9] != '0') { $list_prod1.=  $part[9] * $row[8].";";	} else { $list_prod1.= "; ";}
	IF ($part[10] != "" AND  $part[10] != '0') { $list_prod1.=  $part[10] * $row[8].";";	} else { $list_prod1.= "; ";}
	IF ($part[11] != "" AND  $part[11] != '0') { $list_prod1.=  $part[11] * $row[8].";";	} else { $list_prod1.= "; ";}
	IF ($part[12] != "" AND  $part[12] != '0') { $list_prod1.=  $part[13]."  ; ".$part[12] * $row[8].";";	} else { $list_prod1.= "; ;";}
	IF ($part[14] != "" AND  $part[14] != '0') { $list_prod1.=  $part[15]." ;".$part[14] * $row[8].";";	} else { $list_prod1.= ";; ";}
	IF ($part[16] != "" AND  $part[16] != '0') { $list_prod1.=  $part[16] * $row[8].";";	} else { $list_prod1.= "; ";}
	IF ($part[17] != "" AND  $part[17] != '0') { $list_prod1.=  $part[17] * $row[8].";";	}	 else { $list_prod1.= "; ";}
	IF ($part[18] != "" AND  $part[18] != '0') { $list_prod1.=  $part[18] * $row[8].";";	} else { $list_prod1.= "; ";}
	IF ($list_prod1 != "") {
	...
	if ($idd != ''){
	$query = "INSERT INTO orders (number, DATE_OR, USER_ID, STATUS_ID, CLIENT_ID,CUR_ID) 
	VALUES('{$idd}'	,'{$date_}','{$orderUsers}',{$orderStatus},{$orderClient},'{$val_}');";
	mysql_query($query) or die($query);
	$insert_ID = mysql_insert_id();
	$query = "	UPDATE settings SET 
	val = ''
	...
	VALUES(	'{$date_}','{$orderUsers}',{$orderStatus},{$orderClient},'{$val_}');";
	mysql_query($query) or die($query);
	$insert_ID = mysql_insert_id();
	}	
	echo $insert_ID;
	...
	$query1 = "INSERT INTO order_product (ORDER_ID, PRODUCT_ID, TOTAL, PRICE, SUMM, TEMPLATE, TEMP_PR, SIZE, DIZ, SHABLON_CL, p_names, PR, press_diz, print_diz, sum_press, view_diz, view_press, flags, cshivka, cl_file, units, num_prod_ord) 
	VALUES(	'{$insert_ID_ord}','{$row[1]}','{$row[2]}','{$row[3]}','{$row[4]}','{$row[5]}','{$row[6]}','{$row[7]}','{$row[8]}','{$row[9]}','{$row[10]}','{$row[11]}','{$name_dir3}','{$name_dir2}','','{$row[15]}','{$row[16]}','{$row[17]}','{$row[18]}','{$name_dir1}','{$row[20]}','{$kol}');";
	mysql_query($query1) or die($query1);
	}
	...
	post_region_id, post_post_city, post_post_street, post_post_kor, post_post_kv, post_post_index, dev_region_id, dev_post_city, dev_post_street, dev_post_kor, dev_post_kv,dev_post_index,post_house_num,dev_house_num,dev_raion,post_raion) 
	...
	,'{$dev_post_city}','{$dev_post_street}','{$dev_post_kor}','{$dev_post_kv}','{$dev_post_index}','{$post_post_home}','{$dev_post_home}','{$dev_post_raion}','{$post_post_raion}');";
	...
	$query = "select val from settings where id = 18; ";
	$result = mysql_query($query) or die($query);
	while ($row = mysql_fetch_row($result)) {
	$last_num_ = $row[0];
	}
	...
	post_post_street, post_post_city, post_region_id, post_house_num, dev_house_num, dev_raion, post_raion 
