...
<?
	...
	if ($_GET['id'] == '1'){
		...
		unlink("UpdateTn/Rashod.dbf") ;
		unlink("UpdateTn/Dogovora.dbf") ;
		unlink("UpdateTn/Kontr.dbf") ;
		unlink("UpdateTn/Tovar.dbf") ;
		unlink("UpdateTn/RashodTv.dbf") ;
		
		if (file_exists("UpdateTn/Rashod.dbf") OR file_exists("UpdateTn/Dogovora.dbf") OR file_exists("UpdateTn/Kontr.dbf") OR file_exists("UpdateTn/Tovar.dbf") OR file_exists("UpdateTn/RashodTv.dbf")) {
		...
		//Создем файл
		$DBF_Rashod   = dbase_create('UpdateTn/'.$Rashod, $def_Rashod);
		$DBF_Dogovora = dbase_create('UpdateTn/'.$Dogovora, $def_Dogovora);
		$DBF_Kontr    = dbase_create('UpdateTn/'.$Kontr, $def_Kontr);
		$DBF_Tovar    = dbase_create('UpdateTn/'.$Tovar, $def_Tovar);
		$DBF_RashodTv = dbase_create('UpdateTn/'.$RashodTv, $def_RashodTv);
		
		$path = "UpdateTn/".date("d_m_Y")."/";
		if (!file_exists($path)) {
		mkdir($path, 0700);
		}
		
		$DBF_Rashod1   = dbase_create('UpdateTn/'.date("d_m_Y")."/".$Rashod1, $def_Rashod);
		$DBF_Dogovora1 = dbase_create('UpdateTn/'.date("d_m_Y")."/".$Dogovora1, $def_Dogovora);
		$DBF_Kontr1    = dbase_create('UpdateTn/'.date("d_m_Y")."/".$Kontr1, $def_Kontr);
		$DBF_Tovar1    = dbase_create('UpdateTn/'.date("d_m_Y")."/".$Tovar1, $def_Tovar);
		$DBF_RashodTv1 = dbase_create('UpdateTn/'.date("d_m_Y")."/".$RashodTv1, $def_RashodTv);
		...
		//$query = "select t.dates_, t.num_tm, t.seria, t.val_, c.UNP, c.CLIENT_NAME, c.ADDRESS_DEV, c.CODE_BANK, c.ACCT,  c.num_doc, (select u.USER_FIO from users u where u.USER_LOGIN = t.user_log) user_name,o.NUMBER, DATE_FORMAT(o.DATE_OR, '%d.%m.%Y') date, t.type, t.del, t.id, o.cur_id, t.no_nds   FROM tn_list_par t, orders o, clients c WHERE ((t.del = 0 and t.exp_1c = 0) OR (t.del = 1 and t.exp_1c = 2)) AND NUMBER = t.order_id AND c.ID = o.CLIENT_ID" ;
		$query = "select t.dates_, t.num_tm, t.seria, t.val_, c.UNP, c.CLIENT_NAME, c.ADDRESS_DEV, c.CODE_BANK, c.ACCT,  c.num_doc, (select u.USER_FIO from users u where u.USER_LOGIN = t.user_log) user_name,o.NUMBER, DATE_FORMAT(o.DATE_OR, '%d.%m.%Y') date, t.type, t.del, t.id, o.cur_id, t.no_nds   FROM tn_list_par t, orders o, clients c WHERE NUMBER = t.order_id AND c.ID = o.CLIENT_ID and dates_='2018-10-23'" ;
		$result = mysql_query($query) or die($query);
		WHILE ($row = mysql_fetch_row($result)) { 
		$summ_all = 0;
		$num_doc = "";
		...
		iconv("UTF-8", "cp1251", $row[4]), 
		iconv("UTF-8", "cp1251", $row[9]), iconv("UTF-8", "cp1251",$num_doc), $date_rr,iconv("UTF-8", "cp1251", $row[11]),iconv("UTF-8", "cp1251", $row[12])));
		...
		$query1 = "select id, p_names, price,units from order_product where id = ".$tovar_id;
		...
		dbase_add_record($DBF_Tovar,  array(iconv("UTF-8", "cp1251", $row1[0]) , $p_names,$p_names,"","",iconv("UTF-8", "cp1251", $row1[3])));
		dbase_add_record($DBF_RashodTv, array( iconv("UTF-8", "cp1251", $row[1]) , $tovar_id , $total ,$price,$summ_no_nds,$nds,$sum_nds,$summ)); 
		
		dbase_add_record($DBF_Tovar1,  array(iconv("UTF-8", "cp1251", $row1[0]) , $p_names,$p_names,"","",iconv("UTF-8", "cp1251", $row1[3])));
		dbase_add_record($DBF_RashodTv1, array( iconv("UTF-8", "cp1251", $row[1]) , $tovar_id , $total ,$price,$summ_no_nds,$nds,$sum_nds,$summ)); 
